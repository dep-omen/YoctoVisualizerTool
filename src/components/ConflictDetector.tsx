...
export const ConflictDetector: React.FC<ConflictDetectorProps> = ({ config }) => {
  ...
  return (
    <div className="flex-1 h-full flex flex-col bg-[#0d1117] text-[#c9d1d9] overflow-y-auto custom-scrollbar select-text">
      {/* SECTION 1 — SUMMARY BAR AT THE TOP */}
      <div className="p-4 md:p-6 border-b border-gray-800 bg-[#161b22]/90 backdrop-blur shrink-0">
        <div className="max-w-7xl mx-auto space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {/* 1. Recipe Collisions */}
            <div className="p-3 bg-[#0d1117] border border-amber-500/30 rounded-xl flex items-center gap-3 relative overflow-hidden">
            ...
            {/* 2. Silent Overrides */}
            <div className="p-3 bg-[#0d1117] border border-gray-800 rounded-xl flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-gray-800 border border-gray-700 flex items-center justify-center shrink-0">
                <Layers className="w-4 h-4 text-gray-400" />
              </div>
              <div>
                <div className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">Silent Overrides</div>
                <div className="text-xl font-bold font-mono text-gray-200">{stats.silentOverrides}</div>
              </div>
            </div>

            {/* 3. Critical Conflicts (Equal Priority) */}
            <div className={`p-3 bg-[#0d1117] border rounded-xl flex items-center gap-3 ${
              stats.criticalConflicts > 0 ? 'border-red-500/60 bg-red-950/20 animate-pulse' : 'border-gray-800'
            }`}>
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${
                stats.criticalConflicts > 0 ? 'bg-red-500/20 border border-red-500 text-red-400' : 'bg-gray-800 border border-gray-700 text-gray-500'
              }`}>
            ...
            {/* 4. Orphan BBAppends */}
            <div className="p-3 bg-[#0d1117] border border-purple-500/30 rounded-xl flex items-center gap-3">
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto w-full space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-[#161b22] p-3 rounded-xl border border-gray-800">
          {/* Search Box */}
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-[#0d1117] border border-gray-700 text-xs text-white pl-9 pr-3 py-2 rounded-lg focus:outline-none focus:border-blue-500 transition font-mono placeholder:text-gray-500"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white text-xs px-1"
              >
                ✕
              </button>
            )}
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <span className="text-gray-400 flex items-center gap-1 text-[11px] font-medium mr-1 hidden sm:flex">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition ${
                activeFilter === 'ALL'
                  ...
                  : 'bg-[#0d1117] border border-gray-800 text-gray-400 hover:text-white hover:bg-gray-800'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition flex items-center gap-1.5 ${
                activeFilter === 'CRITICAL'
                  ...
                  : 'bg-[#0d1117] border border-gray-800 text-red-400 hover:bg-red-950/40'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition ${
                activeFilter === 'VERSION BUMP'
                  ...
                  : 'bg-[#0d1117] border border-gray-800 text-blue-400 hover:bg-blue-950/40'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition ${
                activeFilter === 'FORK'
                  ...
                  : 'bg-[#0d1117] border border-gray-800 text-amber-400 hover:bg-amber-950/40'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition ${
                activeFilter === 'ORPHANS'
                  ...
                  : 'bg-[#0d1117] border border-gray-800 text-purple-300 hover:bg-purple-950/40'
              }`}
            >
          ...
          <div className="bg-[#161b22] border border-gray-800 rounded-xl overflow-hidden shadow-lg">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-gray-800 bg-[#0d1117]/80 text-gray-400 uppercase tracking-wider font-semibold text-[10px]">
                    ...
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-gray-500">
                        <div className="flex flex-col items-center justify-center gap-2">
                          ...
                          <span className="text-xs text-gray-500">All recipe names are uniquely defined across configured layers.</span>
                        </div>
                      </td>
                    </tr>
                  ) : (
                        ...
                        <React.Fragment key={item.id}>
                          ...
                          >
                            ...
                            <td className="py-3.5 px-4">
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-950/70 border border-emerald-600/70 text-emerald-400 font-mono font-semibold text-[11px] shadow-sm">
                            ...
                            <td className="py-3.5 px-4 font-mono text-gray-300">
                              {item.winningVersion ? (
                                <span className="px-1.5 py-0.5 bg-[#0d1117] border border-gray-800 rounded text-emerald-300">
                                  {item.winningVersion}
                                </span>
                              ) : (
                            ...
                            <td className="py-3.5 px-4">
                              {item.type === 'CRITICAL' && (
                                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-red-950/90 border border-red-500 text-red-300 font-bold text-[10px] uppercase tracking-wider animate-pulse shadow-sm shadow-red-500/20">
                          ...
                          {isExpanded && (
                            <tr className="bg-[#12161f] border-b border-gray-800">
                              <td colSpan={7} className="p-4 pl-12 space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
                                  {/* Winning Recipe File */}
                                  <div className="p-3.5 bg-[#0d1117] border border-emerald-600/40 rounded-lg space-y-2">
                                    ...
                                    <div className="p-2 bg-[#161b22] rounded border border-gray-800 text-[11px] text-gray-300 break-all flex items-center justify-between gap-2">
                                      <span className="truncate">{item.winningPath}</span>
                                      <button
                                        onClick={(e) => handleCopyPath(item.winningPath, e)}
                                        className="shrink-0 p-1 hover:bg-gray-700 rounded text-gray-400 hover:text-white transition"
                                    ...
                                    <div className="text-[10px] text-gray-400">
                                      Layer: <span className="text-gray-200 font-semibold">{item.winningLayer.name}</span> ({item.winningLayer.path})
                                    </div>
                                  </div>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {item.losingLayers.map((loser, lIdx) => (
                                      <div
                                        key={lIdx}
                                        className="p-3 bg-[#0d1117] border border-rose-800/40 rounded-lg space-y-2"
                                      >
                                        <div className="flex items-center justify-between">
                                        ...
                                        <div className="p-2 bg-[#161b22] rounded border border-gray-800 text-[11px] text-gray-400 break-all flex items-center justify-between gap-2">
                                        ...
                                        <div className="text-[10px] text-gray-400">
                                          Layer: <span className="text-gray-200 font-semibold">{loser.layer.name}</span>
                                        </div>
                                      </div>
                                    ))}
                                  </div>
        ...
        <div className="bg-[#161b22] border border-gray-800 rounded-xl overflow-hidden shadow-lg mt-6">
          <div
            onClick={() => setIsOrphansCollapsed(prev => !prev)}
            className="p-4 bg-[#161b22] border-b border-gray-800 flex items-center justify-between cursor-pointer hover:bg-[#1a2029] transition select-none"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  Append files that do not match any known <code className="text-gray-300 font-mono">.bb</code> recipe in any active layer (targets missing or inactive upstream recipes).
                </p>
              </div>
            </div>
                    ...
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-gray-500">
                        <div className="flex flex-col items-center justify-center gap-1.5">
                          ...
                          <span className="text-[11px] text-gray-500">All .bbappend files cleanly match an active .bb recipe.</span>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    filteredOrphans.map(orphan => (
                      <tr key={orphan.id} className="hover:bg-[#1a2029] transition-colors">
                        <td className="py-3.5 px-4 font-mono font-bold text-purple-300">
                          {orphan.filename}
                        </td>
                        <td className="py-3.5 px-4 font-mono font-semibold text-gray-200">
                          <span className="px-2 py-0.5 bg-gray-800 rounded border border-gray-700 text-gray-300 text-[11px]">
                            {orphan.layer.name}
                          </span>
                        </td>
